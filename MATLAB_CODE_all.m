...
% Save x-axis limits 
if(onlyPlotHist ~= 0)
    originalXLim = max(ax3.XLim);
end
    
% Compute autocorrelation 
untrapLagDuration = untrapDurationSeconds/1000;
untrapLagsPlot = floor(untrapLagDuration / apdData.dt);
    
if(highpassAutocorr == 1)
    highpassFiltHz = 10;
    [acf,lags,bounds] = autocorr(highpass(apdData.data(untrappedIdx:untrappedIdx+untrapDurationSamples),highpassFiltHz,1/apdData.dt),'NumLags',untrapLagsPlot);

else
    [acf,lags,bounds] = autocorr(apdData.data(untrappedIdx:untrappedIdx+untrapDurationSamples),'NumLags',untrapLagsPlot);
end
untrapACF = acf;
untrapLags = lags.*apdData.dt.*1000; % Convert to ms
untrapF = fit(untrapLags,untrapACF,'exp2');
untrapTau = -1/untrapF.d;
    
    
trapLagDuration = trapDurationSeconds/1000;
trapLagsPlot = floor(trapLagDuration / apdData.dt);
    
if(highpassAutocorr == 1)
    [acf,lags,bounds] = autocorr(highpass(apdData.data(trappedIdx:trappedIdx+trapDurationSamples),highpassFiltHz,1/apdData.dt),'NumLags',trapLagsPlot);

else
    [acf,lags,bounds] = autocorr(apdData.data(trappedIdx:trappedIdx+trapDurationSamples),'NumLags',trapLagsPlot);
end
trapACF = acf;
trapLags = lags.*apdData.dt.*1000; % Convert to ms
    
xlim([0 max([untrapLags;trapLags])]);
if(onlyPlotHist == 0)
        
    FitHandle = plot(untrapF,'--');
    set(FitHandle,'color',[lightUntrapColor,lightenFactor], 'LineWidth', 1.5);
    hold on;
        
    % Plot actual autocorr
    FitHandle = plot(untrapLags, untrapACF);
        
    hold on;
    set(FitHandle,'color',plotColors(1,:), 'LineWidth', 1.5);
end
    
    
%% Plotting double exponential fit
trapF = fit(trapLags,trapACF,'exp2');
trapTau = -1/trapF.d;
FitHandle = plot(trapF,'--');
set(FitHandle,'color',[lightTrapColor,lightenFactor], 'LineWidth', 1.5);
hold on;
FitHandle = plot(trapLags, trapACF);
set(FitHandle,'color',plotColors(onlyPlotHist+2,:), 'LineWidth', 1.5);
xlim([0 max([untrapLags;trapLags])]);
